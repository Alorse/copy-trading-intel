"""Pair selection with normalised metrics (equal weight per trader).
No downloads: reads only the flattened CSVs."""
...
for sym, traders in bysym.items():
    ...
    ntr  = len(traders)
    if ntr < 20 or npos < 100:      # minimum sample
        continue
    usd = sum(p for v in traders.values() for _, p in v)
    # normalised metric: mean ROI per trader, then median across traders
...
h = f"{'symbol':<12}{'pos':>7}{'trad':>6}{'USD':>12}{'medROI%':>9}{'%trWin':>9}{'top1%':>8}{'wr%':>7}{'payoff':>8}{'expROI%':>9}"
print(h); print('-'*len(h))
for d in rows[:25]:
...
for d in rows:
    if d['sym'] in ('BTCUSDT','ETHUSDT','XRPUSDT','SOLUSDT'):
        print(f"{d['sym']:<12}{d['npos']:>7}{d['ntr']:>6}{d['usd']:>12,.0f}"
              f"{d['med_roi']*100:>9.2f}{d['pct_win']:>9.1f}{d['top1']:>8.1f}"
              f"{d['wr']:>7.1f}{d['payoff']:>8.2f}{d['exp_roi']*100:>9.2f}")
print(f"\npairs passing the minimum sample (>=20 traders, >=100 pos): {len(rows)}")
