"""Tests the rule set as a FILTER over REAL positions.
Rules fixed looking ONLY at period 1; evaluated on period 2.
The stop is re-simulated from the MAE reconstructed off the OHLC: if the path
touched it, the trade closes there; otherwise the real outcome is used."""
...
print(f"n={len(R)}  P1={len(A)}  P2={len(B)}  cut {dt.datetime.fromtimestamp(CUT/1000, dt.UTC):%Y-%m-%d}\n")

# thresholds fixed with P1 ONLY
m24q=sorted(z['m24'] for z in A)[2*len(A)//3]
m72q=sorted(z['m72'] for z in A)[2*len(A)//3]
maq =sorted(z['ma']  for z in A)[2*len(A)//3]
print(f"P1 thresholds: mom24h>{m24q*100:.2f}%  mom72h>{m72q*100:.2f}%  distMA200>{maq*100:.2f}%\n")

STOP=0.024   # p90 of the winners' MAE, measured on P1

def apply(z, use_stop=True, sl=STOP):
    if use_stop and z['mae'] <= -sl:
        return -sl            # the path touched the stop
    return z['net']

def ev(rows, label, filt=None, use_stop=True):
    g=[z for z in rows if (filt(z) if filt else True)]
    if len(g)<30: print(f"{label}: small sample ({len(g)})"); return
    v=[apply(z,use_stop) for z in g]
    w=[x for x in v if x>0]
    print(f"{label:<34}n={len(g):>5}  mean {st.mean(v)*100:+7.3f}%  median {st.median(v)*100:+7.3f}%"
          f"  wr {len(w)/len(v)*100:5.1f}%  sum {sum(v)*100:+8.1f}%")

for per,rows in (('P1 (in sample)',A), ('P2 (OUT of sample)',B)):
    print(f"--- {per} ---")
    ev(rows, 'all, no stop', None, False)
    ev(rows, 'all, with 2.4% stop', None, True)
    ev(rows, 'R1 high momentum (3 filters)', lambda z: z['m24']>m24q and z['m72']>m72q and z['ma']>maq)
    ev(rows, 'R2 = R1 + leverage <=25x',     lambda z: z['m24']>m24q and z['m72']>m72q and z['ma']>maq and z['lev']<=25)
    ev(rows, 'R3 = R2 + duration >=1h',      lambda z: z['m24']>m24q and z['m72']>m72q and z['ma']>maq and z['lev']<=25 and z['dur']>=1)
    print()

# significance of the final filter on P2
...
def perm(a,b,n=20000):
    ...
    return (c+1)/(n+1)
print(f"P2: filtered n={len(sel)} mean {st.mean(sel)*100:+.3f}%  vs  rest n={len(rest)} mean {st.mean(rest)*100:+.3f}%")
print(f"    permutation on the mean: p = {perm(sel,rest):.4f}")
