"""Ranking multifactor de traders para COPIAR OPERACIONES.
Principio: el PnL en USD es injusto (lo infla cuenta y leverage). Se usa:
 - alpha = retorno menos la mediana de sus pares en el MISMO simbolo-mes-lado
   (separa habilidad de beta: ganar long en un pump no es merito)
 - retorno DESAPALANCADO (movimiento de precio capturado)
 - tendencia temporal (¿mejora o decae?)
 - riesgo de ruina, concentracion, consistencia, copiabilidad"""
...
for r in csv.DictReader(open('binance_positions.csv')):
    ...
    net=pnl/notio                                    # neto sobre notional (fees dentro)
    pr=(acl/ac-1)*(1 if r['side']=='Long' else -1)   # desapalancado
...
# benchmark: mediana de retorno neto por (simbolo, mes, lado) -> alpha
cell=collections.defaultdict(list)
for x in ROWS: cell[(x['sym'],x['mes'],x['side'])].append(x['net'])
...
def slope(pairs):
    """tendencia temporal: pendiente OLS de retorno vs tiempo normalizado."""
...
for tid,v in T.items():
    ...
    if len(v)<40 or len(al)<30: continue          # muestra minima
...
print(f"candidatos con >=40 posiciones y >=30 con benchmark: {len(CAND)}\n")

# score compuesto, pesos explicitos
...
print("score = 3·z(alpha) + 1.5·(alpha>0 en ambas mitades) + 1·(tendencia>0) − 1·(lev>25) − 1.5·(concentracion>40%) − 0.5·(n<60)\n")
