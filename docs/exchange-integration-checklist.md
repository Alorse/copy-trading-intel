...
| KuCoin | **none found** — `positions/history` (`v2/copyTrading/leadHomePage/positions/history`) returned exactly `pageSize` rows up to `pageSize=500` in a live probe (a 213-row and a 260-row trader each came back in ONE page once `pageSize` exceeded their total) and `totalPage`/`totalNum` matched real row counts across the full 165-trader scrape (busiest trader: 2,628 closed rows, `Butterfly04`, paginated cleanly, 0/165 errors) | `pnl` NET by field name, verified over the full 14,414-row universe: median **−11.93 bps** of notional vs. a price-derived gross reconstruction, 94.7% negative (same fee-deducted signature as every other exchange here). `pnlRatio == pnl / posMargin` to a median absolute difference of **4.9e-5** (p90 9.0e-5, n=14,414) — i.e. `pnlRatio` is the LEVERAGED return on margin; the ranking pipeline uses `pnlRatio / leverage` as the de-leveraged basis. ⚠️ **No natural per-row id** on `positions/history` (unlike Bitget's `orderNo`/Bybit's `orderId`) — dedup key is `(leadConfigId, symbol, startTime, endTime)`, verified unique (0 collisions) on a live 260-row single-trader sample. ⚠️ Leaderboard's `totalPnlDate` (30 points) is **misleadingly named**: its last point matches `thirtyDayPnl`, not `totalPnl` — it is the 30-day series, not lifetime; renamed `pnl_series_30d` on read. `ninetyDayPnlDate` (the longest disclosed series, ~89-91 points) is the drawdown-screen basis, normalized by `leadPrincipal` since no 90d RATIO series is disclosed (`leadShow/pnl/history`'s daily `ratio` field blows up to a 6.7e14-fold "equity curve" under naive compounding — deliberately not used) | plain `curl_cffi` (`impersonate='chrome'`) works with NO auth, NO cookies, on every endpoint (leaderboard, history, current, summary) — the cleanest access path of the six exchanges. ⚠️ **`pageNum` is a silent no-op** on the leaderboard endpoint (`v1/copyTrading/leaderboard/query`) — it always re-returns page 1 regardless of the value sent; the real pagination parameter is **`currentPage`**. ⚠️ **"success:false, code:200" trap**: an invalid/gone `leadConfigId` returns HTTP 200 with `{"success":false,"code":"200","data":null}` on every endpoint — the numeric `code` field is useless as a signal, only the boolean `success` field distinguishes a real answer from a not-found trader. ⚠️ **`leadConfigId` type round-trip**: it's an int in every JSONL source but a string after a CSV round-trip (`csv.DictReader`) — a real bug found live 2026-08-30 when an un-cast string `uid` silently broke every cross-check dict lookup keyed by the JSONL's native int, making the drawdown screen reject 100% of the universe until fixed (see `analysis/TOP5_KUCOIN.md`) | 165 traders (`totalNum`, honest) |
